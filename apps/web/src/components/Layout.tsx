...
import { Users, LogIn, BarChart3, LogOut, Menu, X, Shield, Moon, Sun } from 'lucide-react'
          ...
          <div className="flex justify-between h-16">
              ...
              <div className="flex-shrink-0 flex items-center">
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.style.display = 'none'
                    const lightLogo = e.currentTarget.previousElementSibling as HTMLImageElement
                    if (lightLogo) lightLogo.classList.remove('dark:hidden')
                  }}
                />
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">KRC Check-In</h1>
              </div>
            ...
            <div className="hidden sm:ml-6 sm:flex sm:items-center space-x-3">
              ...
              <button
                ...
                title={user?.dark_mode ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
              >
                {user?.dark_mode ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
              </button>
              <button
                onClick={logout}
                className="bg-white dark:bg-gray-800 p-2 rounded-full text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          ...
          <div className="sm:hidden">
            ...
            <div className="pt-4 pb-3 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="mt-3 space-y-1">
                <button
                  onClick={toggleDarkMode}
                  className="block px-4 py-2 text-base font-medium text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 w-full text-left flex items-center"
                >
                  {user?.dark_mode ? <Sun className="w-4 h-4 mr-3" /> : <Moon className="w-4 h-4 mr-3" />}
                  {user?.dark_mode ? 'Light Mode' : 'Dark Mode'}
                </button>
                <button
                  onClick={logout}
                  className="block px-4 py-2 text-base font-medium text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 w-full text-left flex items-center"
